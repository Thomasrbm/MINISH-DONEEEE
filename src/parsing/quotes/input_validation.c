...
/*   Updated: 2025/02/27 19:01:06 by throbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// if any of ft in the "if" is == 1 we print error + return "" to not be = to null
...
char	*validate_input(char *in, t_shell *shell)
{
	...
	if (validate_quotes_token(shell, in))
		return (ft_strdup(""));
	if (!validate_only_backslash_tick(shell, in) && is_only_backslashes_or_backtick(in))
	{
...
// (find_first_quote(in) == 2 && !while_back_slash(in)
// 			&& 
